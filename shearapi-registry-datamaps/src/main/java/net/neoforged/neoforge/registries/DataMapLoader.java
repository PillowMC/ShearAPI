...
import net.pillowmc.shearapi.registries.datamaps.DataMapsLoadedEvent;
...
public class DataMapLoader implements PreparableReloadListener {
    ...
    public void apply() {
        results.forEach((key, result) -> this.apply((IBaseMappedRegistryInjection) registryAccess.registryOrThrow(key), result));

        // Clear the intermediary maps and objects
        results = null;
    }

    private <T> void apply(IBaseMappedRegistryInjection<T> registry, LoadResult<T> result) {
        registry.shearAPI$getDataMaps().clear();
        result.results().forEach((key, entries) -> registry.shearAPI$getDataMaps().put(
                key, this.buildDataMap(registry, key, (List) entries)));
        ShearAPIRuntime.getRuntime().getModBus().post(new DataMapsLoadedEvent<>(registry));
    }
    ...
    private static Map<ResourceKey<? extends Registry<?>>, LoadResult<?>> load(ResourceManager manager, ProfilerFiller profiler, RegistryAccess access, ICondition.IContext context) {
        ...
        access.registries().forEach(registryEntry -> {
            ...
            for (Map.Entry<ResourceLocation, List<Resource>> entry : fileToId.listMatchingResourceStacks(manager).entrySet()) {
                ...
                final var attachment = DataMapManager.getDataMap((ResourceKey) registryKey, attachmentId);
                if (attachment == null) {
