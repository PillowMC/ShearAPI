...
public class LevelAttachmentsSavedData extends SavedData {
    ...
    public static void init(ServerLevel level) {
        var factory = new SavedData.Factory<>(
                ...
                tag -> new LevelAttachmentsSavedData(level, tag), null);
    ...
    public LevelAttachmentsSavedData(ServerLevel level, CompoundTag tag) {
        this.level = level;
        level.getAttachmentHolder().deserializeAttachments(tag);
    }

    @Override
    public CompoundTag save(CompoundTag tag) {
        // Make sure we don't return null
        return Objects.requireNonNullElseGet(level.getAttachmentHolder().serializeAttachments(), CompoundTag::new);
    }
